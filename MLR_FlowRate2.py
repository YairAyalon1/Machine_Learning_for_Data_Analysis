...
# Multi Linear Machine Learning model for flow rate 2 - write also the predicted results
...
print("1. The predictions for Flow Rate 2 is (testing):")
print(y)
print("2. The predicted Flow Rate 2 for the first line of the dataset is:")
print(ml.predict([[19.25, 20.5, 20.2, 20.2,	116.62,	116.08,	27,	27,	2.51, 2.51,	2.5, 0.2, 0.19,	0.06]]))

# Model performances evaluation
MLR2 = r2_score(y_test, y_pred)
MLR2 = np.round(MLR2, 4)

# Results displaying - actual measurements VS. predicted ones
print("3. Model performances evaluation: {}% accuracy".format(MLR2 * 100))
...
for index1, value in enumerate(predictedData):
    worksheet['P{}'.format(index1 + 2)] = np.round(value, 2)
